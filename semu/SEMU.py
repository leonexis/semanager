    ...
    for object in objectcluster:
        for block in list(object.find("CubeBlocks")):
            ...
            if attrib == cget(config, "type_cargo"):
                # LEO - The way items are stored in the save file has changed
                componentContainer = block.find("ComponentContainer")
                if componentContainer == None:
                    l.debug("Could not find ComponentContainer in type_cargo")
                    continue

                components = componentContainer.find("Components")
                if components == None:
                    l.debug("Could not find Components in ComponentContainer")
                    continue

                componentData = components.find("ComponentData")
                if componentData == None:
                    l.debug("Could not find ComponentData in Components")
                    continue

                component = componentData.find("Component")
                if component == None:
                    l.debug("Could not find Component in ComponentData")

                items = component.find("Items")
                if items == None:
                    l.debug("Could not find Items in Component")
                    continue
...
#############################################
def SaveCubeGrid(objectcluster, cluster, savepath):
    clusterPrimaryName = cluster.primaryName
    ...
    # LEO - remove inappropriate characters from filename (SECURITY)
    # Also fixes raid stations with beacons that have "/" in the name
    mininame = re.sub(r"[^a-zA-Z0-9_ ]+", "", mininame)

    if len(mininame) > nameMaxLength:
        mininame = mininame[:nameMaxLength]

    # LEO - use id of first object in cluster instead of undefined variable
    fname = "%s/%s - %s.xml" % (bakfolder, mininame,
        str(cluster.objectNames[0][0]))
            ...
            if ObjectClusterCleanupCheck(objectcluster, args.cleanup_objects[0], cget(config, "type_cleanup_blocks").split(";"), args.remove_npc_ships, clusterNameObject.blockNames, npcidentities) == True: #Cleanup o'clock!
                ...
                if args.object_backup == True:
                    if SaveAfterRemove(objectcluster):
                        # LEO - send entire cluster name object so we can get id
                        SaveCubeGrid(objectcluster, clusterNameObject, args.save_path)
